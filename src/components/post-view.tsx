    ...
    return (
        <div className={cn(
            ...
            <div className={cn(
                        ...
                        <div className="flex flex-col min-h-full">
                            {currentUrl && (
                                <div className="relative w-full aspect-square md:aspect-auto md:flex-1 bg-muted/20">
                                    {mediaUrls.length > 1 && (
                                        <>
                                            <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/90 z-20"><ChevronLeft className="h-10 w-10" /></button>
                                            <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/90 z-20"><ChevronRight className="h-10 w-10" /></button>
                                        </>
                                    )}
                                     {currentType === 'image' ? (
                ...
                {/* НИЖНЯЯ ПЛАШКА ЛЕВОЙ КОЛОНКИ (ТОЛЬКО ДЛЯ МОБИЛОК) */}
                {!isImageExpanded && (
                    <div className="md:hidden flex-shrink-0 sticky bottom-0 w-full p-4 bg-background/95 backdrop-blur border-t border-border flex items-center justify-between z-20">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <button onClick={() => setShowComments(true)} className="flex items-center gap-2 text-foreground">
            ...
            <div className={cn(
                    ...
                    {author && (
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Комментарии</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {/* НИЖНЯЯ ПЛАШКА ПРАВОЙ КОЛОНКИ (ДЛЯ ПК И МОБИЛЬНЫХ КОММЕНТОВ) */}
                <div className="flex-shrink-0 p-4 border-t border-border bg-background z-40">
                    {/* Статистика: теперь видна только здесь на ПК */}
                    <div className="flex items-center gap-6 mb-4">
                        <button onClick={handleLike} className={cn("flex items-center gap-2 transition-colors", isLiked ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
                            <Heart className={cn("h-6 w-6", isLiked && "fill-current")} />
                            <span className="text-sm font-bold">{likeCount}</span>
                        </button>
                        ...
                        <form onSubmit={handleCommentSubmit} className="flex items-end gap-2">
                            <div className="relative flex-1">
                                ...
                                <button
                                    ...
                                    className="absolute right-2 bottom-2 p-1.5 bg-primary text-primary-foreground rounded-xl disabled:opacity-50 transition-all active:scale-95"
                                >
